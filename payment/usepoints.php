...
if($_SESSION["paymentAck"] == "POINTS")
{
    ...
    $insert_row = $mysqli->query("INSERT INTO transactiontable
                ...
                VALUES
                ('$userId','_INIT','POINTS','', $GrandTotal, $ItemTotalPrice, $ShippingFeeAmount, '$procc')
              ");
	...
	$sizeee = "42";
        ...
        $maill = "<li style='border-bottom:1px solid black;list-style: none;width:80%;float: left;margin-left: 5%;margin-right:-40px;'>
				<div style='float: left;padding: 5px;text-align: center;width:30%;display: inline-block;'>
					...
					<img src='http://themeconcept.net/admin/$piccc' width='100' height='150'>
				</div>
				<div style='float: left;padding: 5px;text-align: center;width:30%;display: inline-block;'>
    ...
    if(isset($_SESSION["points"]) && is_numeric($_SESSION["points"][3])){
        ...
        if($cashPointsAmount <= $ItemTotalPrice){
            ...
            if($cashPointTransactionId >= 0){
          		...
          		finalizeCashPointsPayment($mysqli, $userID, $cashPointTransactionId,$transactionID);
...
<div style='width: 700px;padding:0;margin:0;'>
	...
	<div style='height:100%;padding-bottom: 10%;position: relative;'>
		...
		Hi $userrr! , We have received your order <br>and have your items shipped right away.
		</p>
		<br>
		...
		<center>
			...
			<li  style='border-bottom:1px solid black;list-style: none;width:104%;float: left;background: #313232;margin-left:-60px;margin-top:10px;text-align: center;color: white;padding:5px 10px 5px 10px;'>
				 	...
				 	<h4  style='font-family:century gothic;text-align:center;font-weight: normal;color:gray;'>Shipping takes up to 7<br>working days upon purchase.</h4>
			</li>
			<li  style='border-bottom:1px solid black;list-style: none;width:104%;float: left;background:transparent;margin-left: -60px;text-align: center;color: white;padding:5px 10px 5px 10px;'>
				...
				<h4 style='font-family:century gothic;text-align:left;font-weight:bold;color:#313232;'>PAYMENT METHOD:&nbsp;<span style='font-weight: normal;'>Points</span></h4>
				...
				<img src='http://themeconcept.net/admin/img/thankyou2.jpg' alt='Check' width='100%'>
	...
	<br><br>
		...
		$headers  = 'MIME-Version: 1.0' . "\r\n";
        	...
        	$headers .= 'themeconcept.net';
		
		mail($to, $subject, $Message, $headers);

    // Update total price
    if($mysqli->query("UPDATE transactiontable SET `transactionStatus`='PAID', `totalAmount` = '$GrandTotal', 
